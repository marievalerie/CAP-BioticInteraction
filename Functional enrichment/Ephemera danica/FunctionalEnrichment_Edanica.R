#Epehemera danica - Indoor functional enrichment of GO IDs (biological process terms)
...
setwd('C:/Users/mbras/OneDrive/Desktop/Doktorarbeit/Indoor_Genexpression/functional_annotation_eggnog/ephemera_trinity')

##gene2go must be prepared 
GO_ids = read.csv('Trinity_Gene2GO.csv', sep=';', header = F)

##transform to longformat
long_GO <- gather(GO_ids, Gen, IDs, V2:V612)

##remove genes without GO term annotation
...
setwd('C:/Users/mbras/OneDrive/Desktop/Doktorarbeit/Indoor_Genexpression/DESeq/eph_numerical_pred/salmon_trinity')
pest <- as.data.frame(read.csv('DEG_eph_indoor_numerical_predictor_SVA_corrected_conc.csv', header = T, row.names = 1))
setwd('C:/Users/mbras/OneDrive/Desktop/Doktorarbeit/Indoor_Genexpression/functional_annotation_eggnog/ephemera_trinity/enrichment')
...
##run enrichment test: Fishers Exact Test
...
pdf(file= 'GOenrichment_pesticide_downregulated_BP_top15.pdf', height = 8, width = 16)
gg1 <- ggplot(ggdata, aes(x = Term, y = Significant, size = Significant/Expected, fill = -log10(weight01))) +
  ...
  labs(
    title = 'Insecticide effect E. danica -\ndownregulated genes',
  ...
  theme(
    ...
    legend.text = element_text(size = 20, face = "bold"), # Text size
...
pdf(file= 'GOenrichment_pesticide_upregulated_BP_top15.pdf', height = 8, width = 16)
gg1 <- ggplot(ggdata, aes(x = Term, y = Significant, size = Significant/Expected, fill = -log10(weight01))) +
  expand_limits(y = 7) +
  geom_point(shape = 21) +  #shape = 21 allows to fill with other color
  scale_size(range = c(1,10)) +
